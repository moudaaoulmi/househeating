...
public class OnlyMinTempHeaterBehaviour extends Behaviour {
	...
	agents.Agent agent;
	Room room_in;
	Float diff, min_temp_diff, solar_pow_gain;
	ACLMessage mex;
	AID referee;
	...
	public OnlyMinTempHeaterBehaviour(agents.Agent a, long period) {
		super(a);
		this.agent = a;
		turned_ON = false;
		heating = false;
	...
	public void action() {
		...
		min_temp_diff = room_in.getCurrent_temperature()
				- room_in.getMin_temp();
		...
		// Heating OFF
			...
			if ((diff > 2 || min_temp_diff < 2) && !more_power_mex_sent) {
	...
	private Priority getPriority() {
		...
		if (room_in.getCurrent_temperature() < room_in.getMin_temp()) {
